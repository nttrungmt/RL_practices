...
class Agent:
    ...
    """
    def __init__(self, id, brain, render=False, verbose=False, args=None):
        ...
        self.brain = brain
        self.optimizer = torch.optim.Adam(brain.actor_critic.parameters(), lr=LEARNING_RATE)
        self.logdir = './log/acer/'
        if(self.args is not None):
            self.logdir = self.args.logdir
        self.logdir = str(os.path.join(self.logdir,ENVIRONMENT_NAME))
        print('Log dir will be configured at {}'.format(self.logdir))
        if not os.path.exists(self.logdir):
            os.makedirs(self.logdir)
        time_str = time.strftime("%Y%m%d_%H%M%S")
        log_prefix = '{}_{}'.format(self.id, time_str)
        self.writer = SummaryWriter(log_dir=self.logdir, comment=log_prefix)


class DiscreteAgent(Agent):
    def __init__(self, id, brain, render=True, verbose=True, args=None):
        super().__init__(id, brain, render, verbose, args)
...
class ContinuousAgent(Agent):
    ...
    def run(self):
        ...
        for episode in range(MAX_EPISODES):
            ...
            while not end_of_episode:
                ...
                episode_lens.append(leng(trajectory))
                end_of_episode = trajectory[-1].done[0, 0]
                episode_rewards += sum([transition.rewards[0, 0] for transition in trajectory])
                for trajectory_count in range(np.random.poisson(REPLAY_RATIO)):
                    trajectory = self.buffer.sample(OFF_POLICY_MINIBATCH_SIZE, MAX_REPLAY_SIZE)
                    if trajectory:
                        actor_loss, critic_loss, entropy_loss, opc_action_value, opc_advantage = self.learning_iteration(trajectory)
            self.noise.reset()
            if self.verbose:
                print(", episode rewards {:.2f}".format(episode_rewards))
            self.writer.add_scalar('Agent{}/episode_rewards'.format(self.id), episode_rewards, episode)
            self.writer.add_scalar('Agent{}/episode_lens'.format(self.id), np.mean(episode_lens), episode)
            self.writer.add_scalar('Agent{}/actor_loss'.format(self.id), actor_loss, episode)
            self.writer.add_scalar('Agent{}/critic_loss'.format(self.id), critic_loss, episode)
            self.writer.add_scalar('Agent{}/entropy_loss'.format(self.id), entropy_loss, episode)
            self.writer.add_scalar('Agent{}/opc_action_value'.format(self.id), opc_action_value, episode)
            self.writer.add_scalar('Agent{}/opc_advantage'.format(self.id), opc_advantage, episode)
    ...
    def learning_iteration(self, trajectory):
        ...
        self.optimizer.step()
        self.brain.average_actor_critic.copy_parameters_from(self.brain.actor_critic, decay=TRUST_REGION_DECAY)
        return actor_loss, critic_loss, entropy_loss, opc_action_value, opc_advantage
