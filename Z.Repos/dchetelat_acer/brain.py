...
class DiscreteActorCritic(ActorCritic):
    ...
    def __init__(self):
        super().__init__()
        self.input_layer = torch.nn.Linear(STATE_SPACE_DIM, HIDDEN_LAYER_SIZE)
        self.hidden_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, HIDDEN_LAYER_SIZE)
        self.action_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, ACTION_SPACE_DIM)
        self.action_value_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, ACTION_SPACE_DIM)
...
class ContinuousActorCritic(ActorCritic):
    ...
    def __init__(self):
        super().__init__()
        self.policy_input_layer = torch.nn.Linear(STATE_SPACE_DIM, HIDDEN_LAYER_SIZE)
        self.policy_hidden_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, HIDDEN_LAYER_SIZE)
        self.policy_mean_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, ACTION_SPACE_DIM)
        self.policy_logsd = torch.nn.Parameter(np.log(INITIAL_STANDARD_DEVIATION) * torch.ones((1, ACTION_SPACE_DIM)))
        self.value_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, 1)

        self.sdn_state_input_layer = torch.nn.Linear(STATE_SPACE_DIM, HIDDEN_LAYER_SIZE)
        self.sdn_action_input_layer = torch.nn.Linear(ACTION_SPACE_DIM, HIDDEN_LAYER_SIZE)
        self.sdn_hidden_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, HIDDEN_LAYER_SIZE)
        self.sdn_advantage_layer = torch.nn.Linear(HIDDEN_LAYER_SIZE, 1)
...
class Brain:
    ...
    def print_state_dict(self):
        ...
        if(self.actor_critic is not None):
            print("actor_critic's state_dict:")
            for param_tensor in self.actor_critic.state_dict():
                print(param_tensor, "\t", self.actor_critic.state_dict()[param_tensor].size())
        if(self.average_actor_critic is not None):
            print("average_actor_critic's state_dict:")
            for param_tensor in self.average_actor_critic.state_dict():
                print(param_tensor, "\t", self.average_actor_critic.state_dict()[param_tensor].size())
    def save(self,path):
        torch.save({
            'actor_critic_state_dict': self.actor_critic.state_dict(),
            'average_actor_critic_state_dict': self.average_actor_critic.state_dict()
            }, path)
            
    def load(self,path):
        checkpoint = torch.load(path)
        self.actor_critic.load_state_dict(checkpoint['actor_critic_state_dict'])
        self.average_actor_critic.load_state_dict(checkpoint['average_actor_critic_state_dict'])
