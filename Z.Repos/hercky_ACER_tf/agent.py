...
class Agent():
    def __init__(self, name = -1,
        ...
        self.memory = Memory(10000) # each worker has its own memory
    ...
    def train_off_policy(self, sess):
        ...
        opt_feed_dict = {
            ...
            self.u_i : np.asarray(mu_dist).reshape(-1,2*self.ACTION_DIM),
        ...
        with sess.as_default(), sess.graph.as_default():  
            ...
            try:
                while not coord.should_stop():
                    ...
                    print("Eval results at step {}: total_reward {}, episode_length {}".format(global_step, eps_reward, eps_len))
                    print("Total steps taken so far: {}".format(self.global_counter))
                    self.eval_episode_rewards.append(eps_reward)
                    
                    # # Sleep until next evaluation cycle
                    time.sleep(eval_every)
                    
                    # for stopping once condition is met (depends on environments)
                    if(self.FLAGS.env == 'MountainCarContinuous-v0' and np.mean(self.eval_episode_rewards) > 95):
                        coord.request_stop()
                        return
            except tf.errors.CancelledError:
                return
